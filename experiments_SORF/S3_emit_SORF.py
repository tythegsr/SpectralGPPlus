"""S3 EMIT snow70 independent SORF+NIGP (Woodbury RFFGPR).

Third example in the S1/S2/S3 series: trains on
``split_files/emit_data_snow_70to100.nc`` with 10k train / 5k val / remaining
test. One ``RFFGPR`` per QoI with ``rff_sampling='sorf'`` and classic NIGP.

Joint multitask SORF+NIGP lives in ``experiments_RFFMTGPR/S3_emit_MTGPR.py``.
...
# ---------------------------------------------------------------------------
QOI: list[str] | None = ["cwv", "dust", "aot"]
N_TRAIN = 100000
N_VAL = 5000
NUM_RFF = 400
NUM_INITS = 8
...
# Dust: keep log(y+1) for full-test zeros; AOT uses EMIT logit box.
LOG_SCALE_QOI: list[str] | None = ["dust"]
LOGIT_SCALE_QOI: list[str] | None = []
LOG_OFFSETS: dict[str, float] | None = None  # default dust C=1.0 from S2_LOG_OFFSETS
NIGP = True
FREEZE_EPOCH_NIGP = 200
NONDEFAULT_ONLY = True
FILTER_VALID_LABELS = False
INCLUDE_ELEVATION = True
# Off-floor train focuses regression on continuous ISOFIT mass (see Aug24 slice metrics).
OFF_FLOOR_TRAIN_TASKS: list[str] | None = ["aot", "dust"]
TASK_BAND_CONFIG: str | None = (
    "experiments_toa/configs/s2_task_bands_from_corr_fsnow_70to100_Aug12.json"
)
LOG_LEVEL = "INFO"
...
from experiments_toa.s2_constants import S3_LOGIT_BOUNDS
from mtgpr_experiment_utils import DEFAULT_ADAM_KWARGS
from emit_s3_sorf_base import parse_s3_task_names, run_s3_emit_sorf


def run_s3_emit_sorf_entry(**kwargs) -> dict:
    return run_s3_emit_sorf(**kwargs)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="S3 EMIT independent SORF+NIGP (Woodbury RFFGPR; no SVGP)"
    )
    ...
    save_path = args.save_path or (
        f"experiments_SORF/results/Aug24/s3_emit_sorf_{args.num_inits}inits_"
        f"numrff{args.num_rff}_lr{args.lr}{nigp_str}{freeze_str}{elev_str}"
    ...
    print(
        f"S3 EMIT independent SORF  nigp={nigp}  freeze_epoch_nigp={args.freeze_epoch_nigp}  "
    ...
    run_s3_emit_sorf(
        ...
        logit_bounds=dict(S3_LOGIT_BOUNDS),
