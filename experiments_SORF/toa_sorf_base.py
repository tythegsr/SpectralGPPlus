...
def run_toa_sorf(
    ...
    save_path: str | None = "experiments_SORF/results/toa_sorf",
    ...
    """Train SORF-GP on TOA data (two independent scalar models) and evaluate on held-out test points."""
    set_seed(seed)
    if num_sorf is None:
    ...
    title = f"TOA_nTrain{n_train}_nTest{n_test}_sorfD{num_sorf}"
    ...
    for task_idx, task_name in enumerate(TASK_NAMES):
        ...
        model = RFFGPR(x_train, y_tr_fit, num_rff=num_sorf, ard=ard, rff_sampling="sorf")
        trainer = GPTrainer(
            model,
