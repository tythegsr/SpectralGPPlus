"""Evaluate S4 SORF checkpoints on experiments_toa ASD validation NetCDF."""
...
if str(_RFF_DIR) not in sys.path:
    sys.path.insert(0, str(_RFF_DIR))
...
DEFAULT_CHECKPOINT_TITLE = (
    "S4_EMIT_ST_nTrain100000_nVal5000_nTest15000_sorfD600_T8_correctSorfTrue"
)
...
def _s4_script_warp_defaults() -> tuple[list[str] | None, list[str] | None, dict[str, float] | None]:
    from experiments_SORF.S4_emit_SORF import LOGIT_SCALE_QOI, LOG_OFFSETS, LOG_SCALE_QOI

    return LOG_SCALE_QOI, LOGIT_SCALE_QOI, LOG_OFFSETS


def _infer_warps_from_checkpoints(ckpt_dir: Path):
    """Infer log/logit tasks from checkpoint flags; offsets from S4_emit_SORF."""
    log_tasks: list[str] = []
    ...
    for path in sorted(ckpt_dir.glob("checkpoint_*.pt")):
        ...
        if not task:
            continue
        if payload.get("log_grain"):
            log_tasks.append(task)
        if payload.get("logit_cos"):
...
def _load_run_warps(ckpt_dir: Path):
    """Load log/logit warps from ``gp_S4_*.json`` or checkpoint flags."""
    ...
    print("  (no gp_S4_*.json; inferring warps from checkpoints + S4_emit_SORF defaults)")
...
def _available_asd_tasks(
    ...
    *,
    ...
    return [
        ...
        if os.path.isfile(fs_path(_checkpoint_path(ckpt_dir, task, checkpoint_title=checkpoint_title)))
    ]


def _detect_checkpoint_title(ckpt_dir: Path) -> str:
    """Infer shared checkpoint title prefix from files in ``ckpt_dir``."""
...
def evaluate_s4_checkpoints_on_asd(
    ckpt_dir: Path,
    asd_path: Path,
    *,
    device: str = "cuda",
    ...
    task_names = _available_asd_tasks(ckpt_dir, checkpoint_title=title, candidates=requested)
    ...
    for task_name in task_names:
        ...
        bundle = load_toa_stgp_checkpoint(ckpt_path, device=device)
        ...
        row = {
            ...
            # point = median for LOG_SCALE_QOI, mean otherwise
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"Checkpoint title prefix (default: auto-detect or {DEFAULT_CHECKPOINT_TITLE})",
    )
    parser.add_argument(
        "--tasks",
        nargs="+",
        default=None,
        help=f"Subset of tasks with ASD labels (default: {list(TASK_LABEL_MAP)})",
    )
    parser.add_argument(
        "--coszen-override",
        type=float,
        default=None,
        help="Use a fixed coszen aux input for all ASD scenes (e.g. 0.773)",
    )
