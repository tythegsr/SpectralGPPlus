"""
Batch runner for SORF benchmarks (Ackley, Wing, ...).
...
# Insert last-listed dir first on sys.path so experiments_SORF wins over revisions_april
# for shared module names (e.g. load_experimental_data).
for p in (_ROOT, _REV_APRIL_DIR, _SORF_DIR):
    if str(p) not in sys.path:
        sys.path.insert(0, str(p))


def _ensure_sorf_load_experimental_data() -> None:
    """Bind load_experimental_data to experiments_SORF (both trees define this module)."""
    import importlib.util

    target = (_SORF_DIR / "load_experimental_data.py").resolve()
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load SORF data module from {target}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["load_experimental_data"] = mod
    spec.loader.exec_module(mod)


_ensure_sorf_load_experimental_data()

import gpplus
from A1_wing_SORF import run_wing_sorf
from A1_wing_SORF_vs_TabPFN import run_wing_sorf_vs_tabpfn
from A4_ackley_SORF import run_ackley_40d_sorf
from A10_tabpfn1d_abs_x_SORF import run_tabpfn1d_abs_x_sorf
from A11_tabpfn1d_sin_freq_x_SORF import run_tabpfn1d_sin_freq_x_sorf
from A14_tabpfn1d_step_SORF import run_tabpfn1d_step_sorf
from A15_tabpfn1d_x_squared_SORF import run_tabpfn1d_x_squared_sorf
from A18_tabpfn1d_linear_homoscedastic_SORF import run_tabpfn1d_linear_homoscedastic_sorf
from plot_sorf1d_predictions import plot_all_1d
...
ard = True
num_sorf = 100
RESULTS_ROOT = Path(f"experiments_SORF/results_test_pfn_vs_orf/sorf_batch_sorfNum{num_sorf}")
...
EXPERIMENTS: dict[str, dict[str, Any]] = {
    ...
    #     },
    # },
    # "wing_s0_SORF_vs_TabPFN": {
    #     "run_fn": run_wing_sorf_vs_tabpfn,
    #     "subdir": "wing_s0_SORF_vs_TabPFN",
    #     "label": "Wing s0 SORF vs TabPFN slices",
    #     "noise_levels": NOISE_LEVELS,
    #     "kwargs": {
    #         "train_samples_per_source": [100, 0, 0, 0],
    #         "test_samples_per_source": [5000, 0, 0, 0],
...
_TABPFN1D_PROBLEMS = [
    ("abs_x", run_tabpfn1d_abs_x_sorf, 100),
    ("sin_freq_x", run_tabpfn1d_sin_freq_x_sorf, 100),
    ("step", run_tabpfn1d_step_sorf, 100),
    ("x_squared", run_tabpfn1d_x_squared_sorf, 100),
    # ("linear_homoscedastic", run_tabpfn1d_linear_homoscedastic_sorf, 100),
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch SORF experiment runner")
    parser.add_argument(
        "--results-root",
        type=str,
        default=None,
