...
def run_s4_emit_sorf(
    ...
    for task_idx, task_name in enumerate(names):
        ...
        nan_col = np.full(n_te, np.nan, dtype=np.float64)
        ...
        log_sigma_all.append(
            inv.log_sigma.detach().cpu().numpy() if inv.log_sigma is not None else nan_col
        )
        logit_mu_all.append(
            inv.logit_mu.detach().cpu().numpy() if inv.logit_mu is not None else nan_col
        )
        logit_sigma_all.append(
            inv.logit_sigma.detach().cpu().numpy()
    ...
    y_pred_mean_np = np.column_stack(y_pred_mean_all)
    y_pred_mode_np = np.column_stack(y_pred_mode_all)
    log_mu_np = np.column_stack(log_mu_all)
    log_sigma_np = np.column_stack(log_sigma_all)
    logit_mu_np = np.column_stack(logit_mu_all)
    logit_sigma_np = np.column_stack(logit_sigma_all)
    ...
    if save_path:
        ...
        out_npz = save_s2_predictions_npz(
            ...
            y_pred_mode=y_pred_mode_np if warped_plot else None,
            log_mu=log_mu_np if log_task_names else None,
            log_sigma=log_sigma_np if log_task_names else None,
            log_scale_tasks=log_task_names,
            logit_mu=logit_mu_np if logit_task_names else None,
            logit_sigma=logit_sigma_np if logit_task_names else None,
