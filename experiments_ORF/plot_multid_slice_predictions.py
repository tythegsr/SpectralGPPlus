...
def save_gp_tabpfn_marginal_slices(
    *,
    ...
    """
    One PNG per swept dimension: true wing function vs ORF-GP vs TabPFN with 95% PI bands.
    """
    ...
    device_model = next(orf_model.parameters()).device
    saved: list[Path] = []
    ...
    for dim in slice_dims:
        ...
        mean_orf, std_orf = predict_orf_denormalized(
            orf_model,
        ...
        mean_orf_np = mean_orf.detach().cpu().numpy().ravel()
        std_orf_np = std_orf.detach().cpu().numpy().ravel()
        ...
        ax.fill_between(
            x_np,
            mean_orf_np - interval_z * std_orf_np,
            mean_orf_np + interval_z * std_orf_np,
            color="#1B9E77",
            alpha=0.2,
            linewidth=0.0,
            label=f"ORF-GP ±{interval_z}σ",
            zorder=1,
        )
        ax.plot(x_np, mean_orf_np, color="#1B9E77", lw=1.8, label="ORF-GP mean", zorder=3)
