"""1D f(x)=sin(frequency*pi*x) benchmark with GPPlus ORF kernel."""
...
def run_tabpfn1d_sin_freq_x_orf(
    train_size: int = 10,
    ...
    **kwargs,
    ...
    if save_path is None:
        save_path = f"experiments_ORF/results/tabpfn1d_{sin_pi_frequency_problem_name(frequency)}_rff"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="1D sin(frequency*pi*x) with GPPlus ORF")
    ...
    default_save = f"experiments_ORF/results/tabpfn1d_{sin_pi_frequency_problem_name(args.frequency)}_rff"
    run_tabpfn1d_sin_freq_x_orf(
