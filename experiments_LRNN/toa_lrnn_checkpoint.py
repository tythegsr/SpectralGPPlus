"""Save/load TOA LRNNGPR (single-task) checkpoints."""
...
def load_toa_lrnn_checkpoint(path: str | Path, device: str = "cpu") -> ToaLrnnBundle:
    path = Path(path)
    if not path.is_file():
    ...
    act_name = model_config.pop("activation", "Tanh")
    activation = _ACTIVATION_BY_NAME.get(act_name, nn.Tanh)
    model = LRNNGPR(
        train_x,
        train_y,
        hidden_dims=tuple(model_config.get("hidden_dims", (128, 128))),
        feature_rank=int(model_config.get("feature_rank", 128)),
        activation=activation,
        variance_correction=bool(model_config.get("variance_correction", True)),
    )
