"""Marginal slice plots: y vs one input with others fixed (GP ORF + TabPFN + truth)."""
...
def predict_gp_denormalized(
    ...
    *,
    ...
    del chunk_size  # exact GP evaluates full batch
    model.eval()
    mean, _, _, std = evaluate_gp_model(model, X_scaled)
...
def save_gp_tabpfn_marginal_slices(
    *,
    ...
    """
    One PNG per swept dimension: true wing function vs SORF-GP vs TabPFN with 95% PI bands.
    """
    ...
    device_model = next(sorf_model.parameters()).device
    saved: list[Path] = []
    ...
    for dim in slice_dims:
        ...
        mean_sorf, std_sorf = predict_gp_denormalized(
            sorf_model,
        ...
        mean_sorf_np = mean_sorf.detach().cpu().numpy().ravel()
        std_sorf_np = std_sorf.detach().cpu().numpy().ravel()
        ...
        ax.fill_between(
            x_np,
            mean_sorf_np - interval_z * std_sorf_np,
            mean_sorf_np + interval_z * std_sorf_np,
            color="#1B9E77",
            alpha=0.2,
            linewidth=0.0,
            label=f"SORF-GP ±{interval_z}σ",
            zorder=1,
        )
        ax.plot(x_np, mean_sorf_np, color="#1B9E77", lw=1.8, label="SORF-GP mean", zorder=3)
