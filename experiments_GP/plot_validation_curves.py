...
RESULTS_ROOT = Path("experiments_GP/results/gp_batch")
PLOT_OUTPUT_DIR = Path("experiments_GP/results/gp_batch/plots")
...
def _extract_validation_block(metrics: dict) -> dict[str, list[dict]] | None:
    """Top-level or nested under gp_metrics (wing vs TabPFN combined results)."""
    block = metrics.get("validation_metrics_by_init")
    if block:
        return _normalize_by_init(block)
    nested = metrics.get("gp_metrics")
    if isinstance(nested, dict):
        block = nested.get("validation_metrics_by_init")
        if block:
            return _normalize_by_init(block)
    return None
...
def _has_validation_data(metrics: dict) -> bool:
    if metrics.get("monitor_validation") is False:
        return False
    nested = metrics.get("gp_metrics")
    if isinstance(nested, dict) and nested.get("monitor_validation") is False:
        return False
