"""
Batch runner for exact-GP benchmarks (Ackley, Wing, ...).
...
# Insert last-listed dir first on sys.path so experiments_GP wins over revisions_april
# for shared module names (e.g. load_experimental_data).
for p in (_ROOT, _REV_APRIL_DIR, _GP_DIR):
    if str(p) not in sys.path:
        sys.path.insert(0, str(p))


def _ensure_gp_load_experimental_data() -> None:
    """Bind load_experimental_data to experiments_GP (both trees define this module)."""
    import importlib.util

    target = (_GP_DIR / "load_experimental_data.py").resolve()
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load GP data module from {target}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["load_experimental_data"] = mod
    spec.loader.exec_module(mod)


_ensure_gp_load_experimental_data()

import gpplus
from A1_wing_GP import run_wing_gp
from A1_wing_GP_vs_TabPFN import run_wing_gp_vs_tabpfn
from A4_ackley_GP import run_ackley_gp
from A10_tabpfn1d_abs_x_GP import run_tabpfn1d_abs_x_gp
from A11_tabpfn1d_sin_freq_x_GP import run_tabpfn1d_sin_freq_x_gp
from A14_tabpfn1d_step_GP import run_tabpfn1d_step_gp
from A15_tabpfn1d_x_squared_GP import run_tabpfn1d_x_squared_gp
from A18_tabpfn1d_linear_homoscedastic_GP import run_tabpfn1d_linear_homoscedastic_gp
from gp_experiment_utils import sin_pi_frequency_problem_name
from plot_gp1d_predictions import plot_all_1d
...
ard = True
RESULTS_ROOT = Path("experiments_GP/results_test_pfn_vs_gp/gp_batch")
...
EXPERIMENTS: dict[str, dict[str, Any]] = {
    ...
    #     },
    # },
    # "wing_s0_GP_vs_TabPFN": {
    #     "run_fn": run_wing_gp_vs_tabpfn,
    #     "subdir": "wing_s0_GP_vs_TabPFN",
    #     "label": "Wing s0 GP vs TabPFN slices",
    #     "noise_levels": NOISE_LEVELS,
    #     "kwargs": {
    #         "train_samples_per_source": [100, 0, 0, 0],
    #         "test_samples_per_source": [5000, 0, 0, 0],
...
_TABPFN1D_PROBLEMS = [
    ("abs_x", run_tabpfn1d_abs_x_gp, 100),
    ("sin_freq_x", run_tabpfn1d_sin_freq_x_gp, 100),
    ("step", run_tabpfn1d_step_gp, 100),
    ("x_squared", run_tabpfn1d_x_squared_gp, 100),
    # ("linear_homoscedastic", run_tabpfn1d_linear_homoscedastic_gp, 100),
]
...
for _problem, _run_fn, _train_size in _TABPFN1D_PROBLEMS:
    ...
    for _ood_suffix, _margin in (
        ...
        EXPERIMENTS[_key] = {
            "run_fn": _run_fn,
            "subdir": _subdir,
            "label": f"TabPFN1D {_problem_slug} GP" + (" OOD" if _margin > 0 else ""),
            "noise_levels": NOISE_LEVELS,
            "kwargs": {
                **_TABPFN1D_COMMON_KWARGS,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch exact-GP experiment runner")
    parser.add_argument(
        "--results-root",
        type=str,
        default=None,
