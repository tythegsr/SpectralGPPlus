"""
1D exact-GP prediction plots: training points, true curve, GP mean, and 95% prediction intervals.
"""
...
def save_gp1d_prediction_plot(
    ...
    *,
    ...
    gp_label: str = "Exact GP",
) -> Path:
    """Save a 1D exact-GP prediction figure with optional OOD train-domain markers."""
    ...
    fp = out_dir / (f"{stem}_1d_gp_tabpfn.png" if has_tabpfn else f"{stem}_gp1d.png")
...
def plot_from_npz(npz_path: str | Path, out_dir: str | Path | None = None) -> Path:
    ...
    return save_gp1d_prediction_plot(
        data["x_train"],
        data["y_train"],
        data["x_test"],
        data["y_pred"],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Regenerate 1D exact-GP prediction plots from saved npz files")
    parser.add_argument("--results-root", type=str, default="experiments_GP/results/gp_batch")
