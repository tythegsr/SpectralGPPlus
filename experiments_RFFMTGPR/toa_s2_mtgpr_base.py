    ...
    for t, name in enumerate(names):
        ...
        if task_uses_log_scale(name, warps=warps) and inv.log_mu is not None:
            extra = compute_log_scale_extra_metrics(
                ...
                log_offset=warps.log_offset(name) if warps is not None else 0.0,
            )
            for key, value in extra.items():
                per_task[f"{name}_{key}"] = float(value)
            if warps is not None and warps.log_offset(name) > 0.0:
                per_task[f"{name}_log_offset"] = float(warps.log_offset(name))
