...
Loads nested JSON from results_April28 folders, filters to 10Dn/40Dn and noise
0.005/0.05, and produces:
  - GP+ vs TabPFN v2.5 dual violin grids (6 benchmarks)
  - Three-way comparison (GP+, GP+ PE, TabPFN) for Dixon-Price D20 and Rosenbrock D40
...
MODEL_COL_SPACING = 0.92  # distance between model column centers (was 0.72; prevents overlap)
...
def _plot_model_comparison_grid(
    ...
    *,
    ...
    n_cols = len(metrics)
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(8.5 * n_cols, 6.8 * n_rows), squeeze=False)
...
def _build_gp_vs_tabpfn_cost_rows(
    ...
    pfn_prob = _filter_records(pfn_records, problem=problem)

    dx_values = sorted(
        {r["input_dim"] for r in gp_prob + pfn_prob if r.get("input_dim") is not None},
        key=lambda x: x,
    )
    if not dx_values:
        dx_values = [None]
    ...
    for dx in dx_values:
        for dn in TARGET_DNS:
            for noise in NOISE_LEVELS:
                nk = round(float(noise), NOISE_ROUND_DECIMALS)
                for model_label, recs in [("GP+", gp_prob), ("TabPFN v2.5", pfn_prob)]:
                    subset = [
                        r
                        for r in recs
                        if r["train_size"] == dn
...
def _plot_gp_vs_tabpfn_problem(
    ...
    for dx in dx_values:
        ...
        pfn_dx = _filter_records(pfn_prob, input_dim=dx) if dx is not None else pfn_prob

        grouped_gp = _group_by_dn_and_noise(gp_dx, NOISE_LEVELS)
        grouped_pfn = _group_by_dn_and_noise(pfn_dx, NOISE_LEVELS)
        if not grouped_gp and not grouped_pfn:
            continue
        ...
        all_dns = sorted(set(grouped_gp.keys()) | set(grouped_pfn.keys()))
        if all_dns:
            grouped_by_model = {
                "GP+": {d: grouped_gp.get(d, {}) for d in all_dns},
                "TabPFN v2.5": {d: grouped_pfn.get(d, {}) for d in all_dns},
            }
            _plot_comparison_metric_grids(
                grouped_by_model,
                ["GP+", "TabPFN v2.5"],
                ...
                suptitle_suffix="GP+ vs TabPFN v2.5",
            )

    cost_rows = _build_gp_vs_tabpfn_cost_rows(gp_records, pfn_records, problem)
    safe_label = label.replace(" ", "_")
    _render_cost_table_png(
        cost_rows,
        COST_COLUMNS,
        f"{label} — Cost Table (GP+ vs TabPFN v2.5)",
...
def plot_gp_vs_tabpfn(
    ...
    for problem in problem_list:
        ...
        _plot_gp_vs_tabpfn_problem(problem, gp_records, pfn_records, plot_dir)
...
def plot_cost_tables_only(
    ...
    for problem in BENCHMARK_PROBLEMS:
        ...
        _render_cost_table_png(
            cost_rows,
            COST_COLUMNS,
            f"{cfg['label']} — Cost Table (GP+ vs TabPFN v2.5)",
            save_dir / f"cost_table_{safe_label}.png",
        )
...
def load_all_records() -> tuple[list[dict], list[dict], list[dict], list[dict]]:
    ...
    pfn_records = _load_april28_records(
    ...
    print(
        f"[INFO] Loaded {len(gp_records)} GP+ runs, {len(pfn_records)} TabPFN runs, "
        f"{len(pe_dixon)} PE Dixon runs, {len(pe_rosenbrock)} PE Rosenbrock runs"
    )
    return gp_records, pfn_records, pe_dixon, pe_rosenbrock
...
def main() -> None:
    ...
    parser.add_argument(
        "--sections",
        nargs="*",
        default=["gp_vs_tabpfn", "three_way", "cost_tables"],
        choices=["gp_vs_tabpfn", "three_way", "cost_tables"],
    ...
    gp_records, pfn_records, pe_dixon, pe_rosenbrock = load_all_records()
    sections = set(args.sections)

    if "gp_vs_tabpfn" in sections:
        plot_gp_vs_tabpfn(gp_records, pfn_records, plot_dir, problems=args.problems)

    if "three_way" in sections:
        plot_three_way(gp_records, pe_dixon, pe_rosenbrock, pfn_records, plot_dir)

    if "cost_tables" in sections and "gp_vs_tabpfn" not in sections and "three_way" not in sections:
        plot_cost_tables_only(gp_records, pe_dixon, pe_rosenbrock, pfn_records, plot_dir)
