"""
Single-fidelity Wing (s0): train RFF-GP and TabPFN, then plot marginal slice UQ comparisons.
...
def run_wing_rff_vs_tabpfn(
    ...
    save_path: str | None = "experiments_RFF/results/wing_s0_RFF_vs_TabPFN",
    ...
    """
    Train RFF-GP and TabPFN on wing s0, evaluate on test set, and save slice UQ plots.

    run_models: None=both, 'rff'=RFF only, 'pfn'=TabPFN only (slice plots need both).
    """
    ...
    title = (
        f"wing_s0_RFFvsTabPFN_tr{train_samples_per_source}_te{test_samples_per_source}_"
    ...
    print(title)
    print(
        f"RFF-GP D={num_rff}, ARD={ard}, inits={num_inits}, epochs={num_epochs}, "
    ...
    rff_model = None
    rff_metrics: dict | None = None
    if run_models in (None, "rff"):
        print("\n--- RFF-GP training ---")
        ...
        model = RFFGPR(x_train, y_train_rff, num_rff=num_rff, ard=ard)
        trainer = GPTrainer(
            model,
        ...
        rff_computed = compute_metrics(
            y_test_eval.cpu(),
            pred_mean,
            output_std=pred_std,
            lower_95=lower,
            upper_95=upper,
            training_time=rff_train_time,
            prediction_time=rff_pred_time,
        )
        rff_metrics = {
        ...
        if monitor_validation and n_val > 0:
            rff_metrics["monitor_validation"] = True
            rff_metrics["val_fraction"] = val_fraction
            rff_metrics["n_val"] = n_val
            rff_metrics.update(summarize_validation_from_runs(runs, best_run))
        results["rff_metrics"] = rff_metrics
        print(
            f"RFF test RMSE: {rff_computed['RMSE']:.6f}  "
            f"RRMSE: {rff_computed['RRMSE']:.6f}"
        )
    ...
    if plot_slices:
        if rff_model is None or tabpfn_regressor is None:
            print("Slice plots skipped: need both RFF-GP and TabPFN models.")
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Wing s0 RFF-GP vs TabPFN with slice UQ plots")
    parser.add_argument(
        "--train-per-source",
        type=int,
        nargs=4,
    ...
    parser.add_argument("--num-rff", type=int, default=None)
    ...
    parser.add_argument(
        "--save-path",
        type=str,
        default="experiments_RFF/results/wing_s0_RFF_vs_TabPFN",
    )
    ...
    parser.add_argument("--run-models", type=str, default=None, choices=("rff", "pfn"))
    args = parser.parse_args()

    gpplus.config.configure_logger()
    run_wing_rff_vs_tabpfn(
