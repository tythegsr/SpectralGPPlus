"""
Validation loss curves from saved RFF experiment JSON (validation_metrics_by_init).
...
PLOT_OUTPUT_DIR = Path("experiments_RFF/results/rff_batch/plots")
...
def _extract_validation_block(metrics: dict) -> dict[str, list[dict]] | None:
    """Top-level or nested under rff_metrics (wing vs TabPFN combined results)."""
    block = metrics.get("validation_metrics_by_init")
    if block:
        return _normalize_by_init(block)
    rff = metrics.get("rff_metrics")
...
def _plot_all_inits(
    ...
    title = metrics.get("title", "RFF run")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Validation loss curves for RFF batch results")
