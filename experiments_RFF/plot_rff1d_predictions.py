"""
1D RFF prediction plots: training points, true curve, GP mean, and 95% prediction intervals.
"""
...
def save_rff1d_prediction_plot(
    x_train: np.ndarray,
    y_train: np.ndarray,
    x_test: np.ndarray,
    y_pred: np.ndarray,
    ...
    *,
    ...
    """Save a 1D RFF prediction figure with optional OOD train-domain markers."""
...
def plot_from_npz(npz_path: str | Path, out_dir: str | Path | None = None) -> Path:
    ...
    return save_rff1d_prediction_plot(
        data["x_train"],
        data["y_train"],
        data["x_test"],
        data["y_pred"],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Regenerate 1D RFF prediction plots from saved npz files")
    parser.add_argument("--results-root", type=str, default="experiments_RFF/results/rff_batch")
