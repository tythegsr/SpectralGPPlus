"""
Batch runner for RFF benchmarks (Ackley, Wing, ...).
...
# Insert last-listed dir first on sys.path so experiments_RFF wins over revisions_april
# for shared module names (e.g. load_experimental_data).
for p in (_ROOT, _REV_APRIL_DIR, _RFF_DIR):
    if str(p) not in sys.path:
        sys.path.insert(0, str(p))


def _ensure_rff_load_experimental_data() -> None:
    """Bind load_experimental_data to experiments_RFF (both trees define this module)."""
    import importlib.util

    target = (_RFF_DIR / "load_experimental_data.py").resolve()
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load RFF data module from {target}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["load_experimental_data"] = mod
    spec.loader.exec_module(mod)


_ensure_rff_load_experimental_data()

import gpplus
from A1_wing_RFF import run_wing_rff
from A1_wing_RFF_vs_TabPFN import run_wing_rff_vs_tabpfn
from A4_ackley_RFF import run_ackley_40d_rff
from A10_tabpfn1d_abs_x_RFF import run_tabpfn1d_abs_x_rff
from A11_tabpfn1d_sin_freq_x_RFF import run_tabpfn1d_sin_freq_x_rff
from A14_tabpfn1d_step_RFF import run_tabpfn1d_step_rff
from A15_tabpfn1d_x_squared_RFF import run_tabpfn1d_x_squared_rff
from A18_tabpfn1d_linear_homoscedastic_RFF import run_tabpfn1d_linear_homoscedastic_rff
from plot_rff1d_predictions import plot_all_1d
...
from rff_experiment_utils import sin_pi_frequency_problem_name
...
ard = True
num_rff = 200
RESULTS_ROOT = Path(f"experiments_RFF/results_test_val/rff_batch_rffNum{num_rff}")
...
EXPERIMENTS: dict[str, dict[str, Any]] = {
    ...
    #     },
    # },
    # "wing_s0_RFF_vs_TabPFN": {
    #     "run_fn": run_wing_rff_vs_tabpfn,
    #     "subdir": "wing_s0_RFF_vs_TabPFN",
    #     "label": "Wing s0 RFF vs TabPFN slices",
    #     "noise_levels": NOISE_LEVELS,
    #     "kwargs": {
    #         "train_samples_per_source": [100, 0, 0, 0],
    #         "test_samples_per_source": [5000, 0, 0, 0],
...
_TABPFN1D_PROBLEMS = [
    # ("abs_x", run_tabpfn1d_abs_x_rff, 2000),
    # ("sin_freq_x", run_tabpfn1d_sin_freq_x_rff, 2000),
    # ("step", run_tabpfn1d_step_rff, 2000),
    # ("x_squared", run_tabpfn1d_x_squared_rff, 2000),
    # ("linear_homoscedastic", run_tabpfn1d_linear_homoscedastic_rff, 2000),
]
...
for _problem, _run_fn, _train_size in _TABPFN1D_PROBLEMS:
    ...
    for _ood_suffix, _margin in (
        ...
        EXPERIMENTS[_key] = {
            "run_fn": _run_fn,
            "subdir": _subdir,
            "label": f"TabPFN1D {_problem_slug} RFF" + (" OOD" if _margin > 0 else ""),
            "noise_levels": NOISE_LEVELS,
            "kwargs": {
                **_TABPFN1D_COMMON_KWARGS,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch RFF experiment runner")
    parser.add_argument(
        "--results-root",
        type=str,
        default=None,
