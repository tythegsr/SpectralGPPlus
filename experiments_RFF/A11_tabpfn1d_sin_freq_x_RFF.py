"""1D f(x)=sin(frequency*pi*x) benchmark with GPPlus RFF kernel."""
...
def run_tabpfn1d_sin_freq_x_rff(
    train_size: int = 10,
    ...
    **kwargs,
    ...
    if save_path is None:
        save_path = f"experiments_RFF/results/tabpfn1d_{sin_pi_frequency_problem_name(frequency)}_rff"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="1D sin(frequency*pi*x) with GPPlus RFF")
    ...
    default_save = f"experiments_RFF/results/tabpfn1d_{sin_pi_frequency_problem_name(args.frequency)}_rff"
    run_tabpfn1d_sin_freq_x_rff(
