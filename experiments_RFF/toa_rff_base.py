...
def run_toa_rff(
    ...
    save_path: str | None = "experiments_RFF/results/toa_rff",
    ...
    """Train RFF-GP on TOA data (two independent scalar models) and evaluate on held-out test points."""
    set_seed(seed)
    if num_rff is None:
    ...
    title = f"TOA_nTrain{n_train}_nTest{n_test}_rffD{num_rff}"
    ...
    for task_idx, task_name in enumerate(TASK_NAMES):
        ...
        model = RFFGPR(x_train, y_tr_fit, num_rff=num_rff, ard=ard)
        trainer = GPTrainer(
            model,
