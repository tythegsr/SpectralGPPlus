"""S2 11-QoI TOA benchmark with GPPlus RFF kernel (Woodbury inference)."""
...
from toa_s2_rff_base import run_s2_toa_rff


def run_s2_toa_rff_entry(**kwargs) -> dict:
    return run_s2_toa_rff(**kwargs)


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="S2 11-QoI TOA with GPPlus RFF (Woodbury)")
    ...
    parser.add_argument("--noise-var-fraction", type=float, default=0.001)
    ...
    save_path = args.save_path or (
        f"experiments_RFF/results/s2_toa_rff_{args.num_inits}inits_numrff{args.num_rff}_"
        f"lr{args.lr}_noisevarfrac{args.noise_var_fraction}_noisepriorlogscale{args.noise_prior_log_scale}"
    )
