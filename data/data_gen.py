...
def load_data_wing_MV_MF(
    seed: int,
    n_train: dict = {"s0": 200, "s1": 500, "s2": 1000, "s3": 1000},
    n_test: dict = {"s0": 50, "s1": 100, "s2": 200, "s3": 200},
...
def load_data_1D_inverse_cal_MF(
    ...
    if GPP_AVAILABLE:
        continuous_cols, discrete_cols = utils.get_column_types(qual_dict, num_features=2)
    else:
...
def load_data_1D_inverse_MF(
    ...
    # Get column types (no categorical variables for this problem)
    if GPP_AVAILABLE:
        continuous_cols, discrete_cols = utils.get_column_types(qual_dict, num_features=1)
    else:
...
def load_data_ackley(
    ...
    # Get column types (no categorical variables for this problem)
    if GPP_AVAILABLE:
        continuous_cols, discrete_cols = utils.get_column_types(qual_dict, num_features=dim)
    else:
...
def get_data(problem: str, seed: int, save_dir=None, **kwargs):
    ...
    Args:
        problem (str): Problem name ("wing_MV_MF", "buckling_MF", "borehole_MV_MF", "1D_inverse_cal_MF", "1D_inverse_MF", "1D_sin_cal_MF", "ackley", "wing", "wing_simple")
        seed (int): Random seed
