"""Filter EMIT to pixels with softmax snow fraction in [0.70, 1.00].

Default source is emit_data_filtered.nc. Snow fraction is softmax of
z_snow, z_pv, z_npv, z_soil (same 4-way cover softmax as the domain analysis).
"""
...
DEFAULT_SRC = _ROOT / "split_files" / "emit_data_filtered.nc"
DEFAULT_OUT = _ROOT / "split_files" / "emit_data_snow_70to100.nc"
...
FILTER_RULES = (
    "keep softmax(z_snow,z_pv,z_npv,z_soil)[:,0] in [0.70, 1.00]"
)
    ...
    with h5py.File(src_path, "r") as src:
        ...
        with h5py.File(tmp_path, "w") as out:
            ...
            out.attrs["description"] = np.bytes_(
                b"EMIT pixels with softmax snow fraction in [0.70, 1.00]"
            )
