...
class DefaultParameterInitializer(ParameterInitializer):
    ...
    def get_initialization_config(self, name: str, param: torch.Tensor):
        ...
        if "projection_matrix" in name:
            return {"method": "orthogonal", "gain": 1.0, "description": "Matrix encoder projection matrix"}
        ...
        if "outputscale" in name:
            return {"method": "uniform", "lower": 0.1, "upper": 2.0, "description": "Kernel outputscale"}

        # Likelihood noise
        if "raw_noise" in name or "noise" in name:
            return {"method": "uniform", "lower": -6.0, "upper": -3.0, "description": "Likelihood noise"}
