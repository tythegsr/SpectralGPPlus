...
class MollifiedUniformPrior(Prior):
    ...
    def log_prob(self, X):
        ...
        return (
            Normal(loc=torch.zeros_like(self.a), scale=self.tail_sigma).log_prob(tail_dist)
    ...
    def expand(self, expand_shape):
        ...
        return MollifiedUniformPrior(
            self.a.expand(batch_shape), self.b.expand(batch_shape), self.tail_sigma.expand(batch_shape)
        )
