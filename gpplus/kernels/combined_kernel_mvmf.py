...
class CombinedKernel_MVMF(gpytorch.kernels.Kernel):
                ...
                for i, cat_group in enumerate(cat_cols):
                    ...
                    if isinstance(cat_encoder, str):
                        ...
                        if hasattr(encoder, 'input_size') and encoder.input_size != len(cat_group):
                            raise ValueError(f"Encoder {i} has input_size {encoder.input_size} but group {i} has {len(cat_group)} columns")
                        elif hasattr(encoder, 'num_classes') and encoder.num_classes != len(cat_group):
                            raise ValueError(f"Encoder {i} has num_classes {encoder.num_classes} but group {i} has {len(cat_group)} columns")
                    else:
    ...
    def forward(self, x1, x2=None, diag=False, **kwargs):
        device = x1.device
        cont_cols_tensor = torch.tensor(self.cont_cols, dtype=torch.long, device=device) if (self.cont_cols is not None and len(self.cont_cols) > 0) else torch.tensor([], dtype=torch.long, device=device)
        source_cols_tensor = torch.tensor(self.source_cols, dtype=torch.long, device=device) if (self.source_cols is not None and len(self.source_cols) > 0) else torch.tensor([], dtype=torch.long, device=device)
        ...
        total_cat = sum(len(group) for group in self.cat_cols) if self.has_cat and self.multiple_groups else (len(self.cat_cols) if self.has_cat else 0)
        ...
        if self.has_cont:
            k_cont = self.cont_kernel(x1.index_select(-1, cont_cols_tensor), x2.index_select(-1, cont_cols_tensor), diag=diag, **kwargs)
            result = k_cont
        else:
        ...
        if self.has_source:
            use_eps = isinstance(self.source_encoder, OneHotToLatent) and \
                      getattr(self.source_encoder, "is_probabilistic", True)
            if use_eps:
