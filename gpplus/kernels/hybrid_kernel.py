    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.scaled_rbf_kernel = ScaleKernel(
            self.rbf_kernel, outputscale_constraint=gpytorch.constraints.Interval(1e-6, 1e4)
        )
        ...
        if self.combination_method == "additive":
            self.hybrid_kernel = AdditiveKernel(self.linear_kernel, self.scaled_rbf_kernel)
        else:  # product
            self.hybrid_kernel = ProductKernel(self.linear_kernel, self.scaled_rbf_kernel)
        ...
        if not diag:
            ...
            if hasattr(result, "add_diag"):
                ...
                if hasattr(result, "to_dense"):
                    ...
                    result_dense.add_(
                        torch.eye(result_dense.size(-1), device=x1.device, dtype=result_dense.dtype) * self.jitter
                    )
                    result = result_dense
                else:
                    ...
                    try:
                        ...
                        print(f"Warning: jitter addition failed: {e}")
                        # optional: logging.warning(...) instead of print
                        return result
